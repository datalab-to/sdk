...
@click.command()
@click.argument("path", type=click.Path(exists=True))
@click.option("--page_schema", required=False, help="JSON schema for structured extraction (must contain 'properties' key). Mutually exclusive with --schema_id.")
@click.option("--schema_id", help="ID of a saved extraction schema (e.g. sch_k8Hx9mP2nQ4v). Mutually exclusive with --page_schema.")
@click.option("--schema_version", type=int, help="Version of the saved schema to use. Only valid with --schema_id.")
...
def extract(
    ...
    """Extract structured data from documents using a JSON schema or saved schema ID"""
